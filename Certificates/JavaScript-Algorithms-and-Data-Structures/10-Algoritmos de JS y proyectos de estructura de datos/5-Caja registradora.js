...
function checkCashRegister(price, cash, cid) {
  ...
  const monedasConDinero = cidConImportes
    .filter((dinero) => dinero[1] !== 0)
    .reverse(); // Filtramos las monedas que tengan dinero (que su valor no sea 0)
  let monedasConDineroAUsar = monedasConDinero.filter(
    (moneda) => moneda[2] <= valorDeCambioADar && moneda[1] !== 0
  ); // Filtramos las monedas que su valor sea meno o igual a el vuelto a dar

  let total = Number(
    monedasConDineroAUsar
      .map((moneyBill) => moneyBill[1])
      .reduce((a, b) => a + b)
      .toFixed(2)
  ); // La sumatoria de los importes de todas las monedas que filtramos para usar
  ...
  // La plata sobra para el cambio
    ...
    while (monedasConDineroAUsar.length !== 0) {
      // Mientras que exista monedas para usar realizamos el loop
      ...
      monedasConDineroAUsar[0][1] = Number(
        (monedasConDineroAUsar[0][1] - monedasConDineroAUsar[0][2]).toFixed(2)
      ); // Descontamos del total del importe de la moneda
      valorDeCambioADar = Number(
        (valorDeCambioADar - monedasConDineroAUsar[0][2]).toFixed(2)
      ); // pagamos una parte del vuelto

      // Debemos actualizar las monedas q podemos usar segun lo que falte del vuelto
      monedasConDineroAUsar = monedasConDineroAUsar.filter(
        (moneda) => moneda[2] <= valorDeCambioADar && moneda[1] !== 0
      );

      //  Agregamos un nuevo billete al vuelto o modificamos el valor de un billete
      if (monedasConDineroAUsar.length === 0) {
        // La condicion sera verdadera cuando no exista mas billeter por utilizar
        montosVuelto.push([monedaActual, vueltoMoneda]); // Pusheamos la ultima moneda con su importe
      } else {
        if (monedasConDineroAUsar[0][0] !== monedaActual) {
          // Cuando exista un cambio de moneda
    ...
    if (valorDeCambioADar !== 0) {
      // No tenemos las moendas correctar para poder dar el vuelto
      cambio.status = "INSUFFICIENT_FUNDS";
      cambio.change = [];
      cid = cid; // restablecemos las monedas usadas (no lo pide el programa)
    } else {
      // Tenemos las monedas correctas para el vuelto
      cambio.status = "OPEN";
